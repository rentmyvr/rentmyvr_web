...
export default function FilesPreview({ showList = false, files, onRemove, type, showPreview = () => {} }) {
  ...
  return (
    <List
      ...
      sx={{
        ...
        ...(type === 'STANDARD' && { width: 'calc(100% - 84px)' })
      }}
    >
      {files.map((file, index) => {
        ...
        if (showList) {
          return (
            ...
            >
              {type?.includes('image') && (
                <CardMedia onClick={() => showPreview(file)} component="img" alt="preview" src={preview} style={{ width: '100%' }} />
              )}
              {!type?.includes('image') && <FileFilled onClick={() => showPreview(file)} style={{ width: '100%', fontSize: '1.5rem' }} />}
